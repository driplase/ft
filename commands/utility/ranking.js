	...
	data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand => 
            subcommand.setName('xp')
                .setDescription('Shows xp ranking. (same as /leaderboard)')
                .addIntegerOption(option =>
                    option.setName('page')
                        .setDescription('The leaderboard page number.')
                        .setRequired(false)))   
        .addSubcommand(subcommand => 
            subcommand.setName('streak')
                .setDescription('Shows login streak ranking.')
                .addIntegerOption(option =>
                    option.setName('page')
                        .setDescription('The leaderboard page number.')
                        .setRequired(false))),
	async execute(interaction) {
        try {
            ...
            if (type === "xp") {
                ...
                const m = JSON.parse(fs.readFileSync('./data/lb/streak.json'));
                const g = []
                for (let i in m) {
                    g.push({
                        id: m[i].id,
                        streak: getLoginSteak(m[i].id),
                        l2: getStat(m[i].id).lastLogin
                    })
                }
                g.sort((a, b) => b.streak - a.streak)
                const p = g.findIndex(i => i.id === u);
                const f = g.slice(16 * (pg - 1), pg * 16);
                if (p < 16 * (pg - 1) && p !== -1) {
                    const s = getStat(u);
                    ...
                    r = `-# ___\\#${p + 1} | <@${u}>${badgestring} ${x('command.ranking.streak.loginStreak', lang).replace('%d', g[p].streak.toLocaleString())}___\n`
                }
                for (let i = 0; i < f.length; i++) {
                    console.log(f[i])
                    const s = getStat(f[i].id);
                    ...
                    if (i === p - 16 * (pg - 1)) {
                        r = `${r}${X}___\\#${i + 1 + 16 * (pg - 1)} | <@${f[i].id}>${badgestring} \`${x('command.ranking.streak.loginStreak', lang).replace('%d', f[i].streak.toLocaleString())}\`___${(i + 16 * (pg - 1) === 2) ? "**" : ""}\n`;
                    } else {
                        r = `${r}${X}\\#${i + 1 + 16 * (pg - 1)} | <@${f[i].id}>${badgestring} \`${x('command.ranking.streak.loginStreak', lang).replace('%d', f[i].streak.toLocaleString())}\`${(i + 16 * (pg - 1) === 2) ? "**" : ""}\n`;
                    }
                }
                if (p > 16 * pg) {
                    const s = getStat(u);
                    ...
                    r = `${r}-# ___\\#${p + 1} | <@!${u}>${badgestring} \`${x('command.ranking.streak.loginStreak', lang).replace('%d', g[p].streak.toLocaleString())}\`___\n`
                }
            }
            r.replace(/\n$/, '');
            const m = new EmbedBuilder()
                ...
                .setColor(type === "streak" ? 16737805 : 1744112)
            ...
            console.error(e)
            await interaction.editReply({
                content: "Failed to get the Leaderboard... :("
            })
        }
	},
};