...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('ranks')
		.setDescription('Shows the Lynst Leaderboard.')
	...
	async execute(interaction) {
        try {
            ...
            const g = JSON.parse(fs.readFileSync('./data/lb/lynst.json'));
            ...
            if (p < 16 * (pg - 1) && p !== -1) {
                ...
                r = `-# ___\\#${p + 1} | <@${u}>${badgestring} / ${ix(`rank${getRank(g[p].id)}`)} ${g[p].lynsts?.toLocaleString() ?? 0} ${ix('lynst')}___\n`
            }
            for (let i = 0; i < f.length; i++) {
                ...
                if (i === p - 16 * (pg - 1)) {
                    r = `${r}${X}___\\#${i + 1 + 16 * (pg - 1)} | <@${f[i].id}>${badgestring} / ${ix(`rank${getRank(f[i].id)}`)} ${f[i].lynsts?.toLocaleString() ?? 0} ${ix('lynst')}___${(i + 16 * (pg - 1) === 2) ? "**" : ""}\n`;
                } else {
                    r = `${r}${X}\\#${i + 1 + 16 * (pg - 1)} | <@${f[i].id}>${badgestring}  / ${ix(`rank${getRank(f[i].id)}`)} ${f[i].lynsts?.toLocaleString() ?? 0} ${ix('lynst')}${(i + 16 * (pg - 1) === 2) ? "**" : ""}\n`;
                }
            }
            if (p > 16 * pg) {
                const s = getStat(u);
                ...
                r = `${r}-# ___\\#${p + 1} | <@!${u}>${badgestring} / ${ix(`rank${getRank(g[p].id)}`)} ${g[p].lynsts?.toLocaleString() ?? 0} ${ix('lynst')}___\n`
            }
            r.replace(/\n$/, '');
